...
test("User API - creation fails with proper statuscode and message if username already taken", async () => {
  const usersAtStart = await helper.usersInDb();
...
test.only("POST /api/blogs/:id/comments - a comment can be added to a blog", async () => {
  const blogsAtStart = await helper.blogsInDb();
  ...
  assert.strictEqual(updatedBlog.comments.length, blog.comments.length + 1);
  assert(updatedBlog.comments.includes("This is a new comment"));
});
